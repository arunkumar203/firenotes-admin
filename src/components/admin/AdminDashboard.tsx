...
const AdminDashboard = (): ReactElement => {
  ...
  useEffect(() => {
    const loadUsers = async (): Promise<void> => {
      try {
        setLoading(true);
        const userList = await getAllUsers() as Array<Record<string, unknown>>;
        ...
        const validUsers = userList
          .map(user => {
            const userProfile: UserProfile = {
              ...
              role: (user.role || 'user') as UserRole, // Default to 'user' if role is not set
          ...
          .filter(user => user.uid) // Filter out any users without a uid
          .filter(user => {
            // Ensure the user has a valid role
            return ['user', 'admin', 'root_admin'].includes(user.role);
          });
                  ...
                  <tr key={userItem.uid}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {user?.role === 'root_admin' ? (
                        ...
                        // For non-root admins, just show the role as text

...
export const useNotes = (folderId?: string | null): UseNotesReturn => {
  ...
  // Effect for fetching notes when key parameters change
  useEffect(() => {
    if (!user) {
      setNotes([]);
      setLoading(false);
    ...
    // Track note IDs to prevent duplicates
    ...
    const fetchNotes = async () => {
      ...
      try {
        ...
        // For 'all' or no folder selected, fetch all user's notes
        ...
        if (snapshot.exists()) {
          snapshot.forEach((childSnapshot) => {
            ...
            if (noteData.userId === user.uid && !noteIds.has(noteId)) {
              noteIds.add(noteId);
              notesData.push({
                ...noteData,
                id: noteId
        ...
        // If a specific folder is selected, filter notes by folder
        if (folderId && folderId !== 'all') {
          ...
          if (Array.isArray(folderData.noteIds) && folderData.noteIds.length > 0) {
    ...
    const handleNotesUpdate = (snapshot: DataSnapshot) => {
      if (!isMounted) return;
      ...
      // If we're viewing a specific folder, filter the notes
      if (folderId && folderId !== 'all') {
        const folderRef = ref(db, `folders/${folderId}`);
        get(folderRef).then((folderSnapshot) => {
          if (!isMounted) return;
    ...
    // Set up real-time listener
    const unsubscribe = onValue(notesRef, handleNotesUpdate, (err) => {
      console.error('Error in real-time update:', err);
      if (isMounted) {
        setError('Real-time update failed');
        toast.error('Failed to update notes in real-time');
      }
    });
